-- =============================================
-- Migration: Create public.iniciar_lavagem()
-- Date: 2026-09-15
-- Description: RPC de início de lavagem para usuários
-- ativos com perfil ADMIN ou LAVADOR. Transição permitida
-- apenas RESERVADA -> EM_LAVAGEM, com lock de linha
...
CREATE OR REPLACE FUNCTION public.iniciar_lavagem(
  p_reserva_id uuid,
...
BEGIN
  ...
  -- 4. Transição permitida: somente RESERVADA -> EM_LAVAGEM
  IF v_status_atual <> 'RESERVADA' THEN
    RAISE EXCEPTION 'A lavagem não pode ser iniciada no status atual.'
      USING HINT = 'Somente reservas com status RESERVADA podem iniciar lavagem.';
  END IF;

  -- 5. Início da lavagem
  UPDATE public.reservas_lavagem r
  SET status = 'EM_LAVAGEM'
  WHERE r.id = p_reserva_id;

  -- 6. Auditoria (atômica com o início)
...
-- Grants
-- =============================================
REVOKE EXECUTE ON FUNCTION public.iniciar_lavagem(uuid, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.iniciar_lavagem(uuid, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.iniciar_lavagem(uuid, text) FROM service_role;
GRANT EXECUTE ON FUNCTION public.iniciar_lavagem(uuid, text) TO authenticated;
